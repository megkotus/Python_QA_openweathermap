...
@pytest.mark.parametrize(
    'city', ['London', 'Podgorica', 'Berlin', 'Dubrovnik']
)
def test_weather_api_returns_correct_city_name(city):
    ...
    assert (
        json_data['name'] == city
    ), f"Expected {city}, but got {json_data['name']}"
...
def test_weather_api_returns_correct_city_by_id(city):
    ...
    assert (
        response.json()['name'] == city
    ), f'Expected {city}, got {response.json()["name"]}'
